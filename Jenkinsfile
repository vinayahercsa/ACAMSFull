  ...
  stage('checkout source') {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
			...
			if (isUnix()) {
                sh 'ls -la'
				rmsg = sh returnStdout: true, script: "${toolbelt}/sf project deploy validate --verbose -x manifest/package.xml -l NoTestRun -o ${sf_username}"
//sh "${toolbelt}/sfdx plugins:install @salesforce/sfdx-scanner"
         // sh "export SFDX_SCANNER_NO_PROMPT=true && ${toolbelt}/sfdx scanner:run --target force-app/main/default/classes/ACAMSAccountTriggerHandler.cls --json "
			   ...
			   rmsg = bat returnStdout: true, script: "${toolbelt}/sf project deploy validate --verbose -x manifest/package.xml -l NoTestRun -o ${sf_username}"
			}
